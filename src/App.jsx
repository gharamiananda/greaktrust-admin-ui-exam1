...
function App() {
  ...
  if(!loading &&  (!users.length || !users)){
    content=<tr><td colSpan={5}><h3>No Items found!</h3></td></tr>
  }
else if(!loading && users.length > 0 ){
  content=  users.map((user, ind) => (
    <tr key={user.id}>
      <td>
      ...
      <input name={user.id} onChange={(e) => handleChange(e, user.id)} />
      ...
      <Table striped bordered hover>
        <thead>
          <tr>
            <th>
              <input 
              disabled={!users.length || !users}
              type="checkbox" name='all' checked={users.filter(user => user.isChecked !== true).length < 1} onChange={(e) => handleChange(e, "all")} />
            </th>
            <th>Name</th>
            <th>Email</th>
            <th>Role</th>
